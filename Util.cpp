...
float lArm[4] = { 70, 110, 70, 110 };
int giLArmDirection[4] = { 1,1,1,1 };

float rArm[4] = { 110, 70, 110, 70 };
...
void ApplyTexture(GLuint texture, float vertice1[3], float vertice2[3], float vertice3[3], float vertice4[3])
{
...
void DrawChest(GLuint texture, float color[3], float bottomColor[3])
{
	...
	DrawCubiod(pointsChestBottom, bottomColor);

	ApplyTexture(texture, points[3], points[2], points[6], points[7]);
}

void DrawFace(GLuint texture)
{
...
void DrawCircle(float radius)
{
	...
	glLineWidth(2);
	glBegin(GL_POLYGON);
	for (angle = 0.0f; angle < 2 * 3.145; angle = angle + 0.01f)
...
void DrawSector(float aRadius, float bRadius, float z, float color[3], float minAngle, float maxAngle)
{
	...
	for (angle = minAngle; angle <maxAngle; angle = angle + 0.01f)
...
void DrawCylinder(float height, float radius, float color[3])
{
	...
	glColor3f(color[0], color[1], color[2]);
	gluCylinder(quadric, radius, radius, height, 10, 10);
...
void DrawLegoCharacters(GLuint textureChest[4], GLuint textureFace[4])
{
	int i = 0;
	...
	for (i = 0; i < 4; i++)
	{
		...
		glPushMatrix();
		DrawHead(textureFace[i]);
		glPopMatrix();

		glTranslatef(0.0f, -gfNeckHeight, 0.0f);

		DrawChest(textureChest[i], chestColor[i], bottomColor[i]);
...
void DrawSingleLegoCharacters(int currentLegoCharacter)
{
	int i = currentLegoCharacter;
	float translateX = gfTranslateX;
	float color[3] = { 0.0f, 0.0f, 0.0f };
